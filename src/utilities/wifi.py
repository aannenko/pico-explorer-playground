...
ATTEMPT_TIMEOUT_MS = const(10_000)
CONNECTING_SLEEP_MS = const(200)
...
def try_connect(ssid: str, password: str, timeout_ms: int = 300_000) -> bool:
    ...
    if timeout_ms < ATTEMPT_TIMEOUT_MS:
    ...
    attempt_start_ms = time.ticks_add(now_ms, -ATTEMPT_TIMEOUT_MS)
    ...
    while True:
        ...
        if status == STAT_CONNECTING:
            if time.ticks_diff(now_ms, attempt_start_ms) >= ATTEMPT_TIMEOUT_MS:
                wlan.disconnect()
