...
class FakePicoGraphics:
    ...
    def measure_text(self, text: str, scale: int) -> int:
        # Device-observed measurements (bitmap8, scale=3) — see
        # `files/brainstorm.md` "Device-observed measure_text data points".
        # The table covers every string the layout / formatter touches; the
        # fallback formula is only hit by ad-hoc test inputs.
        if scale == 3:
            widths = {
                "8888": 57,
                "9999": 57,
            ...
            if text in widths:
                return widths[text]
        # Deterministic fallback so tests can derive layout invariants
        # without hardcoding magic numbers for non-canonical samples.
        return max(1, len(text) * (scale * 4 + 2))

    def create_pen(self, r: int, g: int, b: int) -> int:
        self.calls.append(("create_pen", (r, g, b), {}))
        # Deterministic, distinct, easy to read back.
        return (r << 16) | (g << 8) | b
