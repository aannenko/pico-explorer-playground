...
- Sensor dashboard view (**partially implemented**): shows BME690 readings (temperature, pressure, humidity, gas resistance, heater status) and a header with local time (via `TimeService.now`). `PimoroniBME690` owns a dedicated `machine.Timer` so its reads happen on a strict cadence — the gas-resistance sensor is sensitive to inter-read spacing and must not share cadence with the general-purpose `TickScheduler`. History graphs are planned but not yet implemented. Show current time/date at the top.
